...
The typical workflow is to train a segmentation model, run inference to obtain probability maps and uncertainties, then use the graphical interface to rapidly label clusters of points. The exported annotations can be fed back into the training scripts for iterative improvement.
...
python gui_main.py
```
A start-up dialog lets you continue a previous session, load an existing project or create a new one from a predictions database. Label clusters of points and export the results when finished.
